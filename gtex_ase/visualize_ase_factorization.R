...
# Input files
covariate_file <- paste0(processed_data_dir, "tissue_subset_10_annotated_sample_ids.txt")
covariates <- read.table(covariate_file, header=TRUE, sep="\t", comment.char="",quote="")


loading_file <- paste0(eqtl_results_dir, "tissue_subset_10_k_10_ase_factorization_temper_U.txt")
loadings <- read.table(loading_file, header=FALSE)


output_stem <- "tissue_subset_10_k_10_"
