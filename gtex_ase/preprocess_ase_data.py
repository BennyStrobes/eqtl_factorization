...
def generate_raw_ase_matrix(ase_input_data_dir, tissue_names, unfiltered_samples_file, unfiltered_ase_file):
	...
	for individual_ase_file in os.listdir(ase_input_data_dir):
		...
		for line in f:
			...
			if low_mapability == '1' or mapping_bias_sim == '1' or genotype_warning == '1':
				continue
			if site_id not in site_to_gene:
				site_to_gene[site_id] = gene_id
			if gene_id != site_to_gene[site_id]:
				print('assumption errorr!')
				pdb.set_trace()
			# Add ASE INFO
			full_site_name = site_id + ':' + gene_id
			if full_site_name not in sites:
				sites[full_site_name] = ['NA']*num_samples
			sample_position = sample_id_to_position[sample_id]
			sites[full_site_name][sample_position] = ref_count + '/' + total_count
		f.close()
...
gtex_individual_information_file = sys.argv[3]
cell_type_decomposition_hlv_file = sys.argv[4]
output_root = sys.argv[5]



# Extract dictionary list of tissue names
tissue_names = extract_tissue_names(tissues_file)
...
# rows are het sites
unfiltered_samples_file = output_root + 'raw_sample_ids.txt'
#generate_raw_ase_sample_names(ase_input_data_dir, tissue_names, unfiltered_samples_file)


annotated_samples_file = output_root + 'annotated_sample_ids.txt'
annotate_sample_ids(unfiltered_samples_file, annotated_samples_file, gtex_individual_information_file, cell_type_decomposition_hlv_file, tissue_names)
...
#generate_raw_ase_matrix(ase_input_data_dir, tissue_names, unfiltered_samples_file, unfiltered_ase_file)
