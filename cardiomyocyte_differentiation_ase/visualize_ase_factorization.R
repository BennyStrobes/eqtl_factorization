...
make_loading_boxplot_for_one_factor_by_categorical_covariate <- function(covariate, real_valued_covariate, loadings, factor_number, covariate_name) {
	df <- data.frame(loading=loadings, covariate=factor(covariate), real_valued_covariate=real_valued_covariate)

	boxplot <- ggplot(df, aes(x=covariate, y=loading, fill=real_valued_covariate)) + geom_boxplot(outlier.size = .00001) +
				gtex_v8_figure_theme() + 
...
make_loading_boxplot_plot_with_row_for_every_factor_by_categorical_covariate <- function(covariate, real_valued_covariate, loadings, covariate_name) {
	loading_vec <- c()
	covariate_vec <- c()
	num_factors <- dim(loadings)[2]

	factor_number <- 1
	factor_1_boxplot <- make_loading_boxplot_for_one_factor_by_categorical_covariate(covariate, real_valued_covariate, loadings[,factor_number], factor_number, covariate_name)

	factor_number <- 2
	factor_2_boxplot <- make_loading_boxplot_for_one_factor_by_categorical_covariate(covariate, real_valued_covariate, loadings[,factor_number], factor_number, covariate_name)
...
make_loading_expression_pc_scatter_for_each_factor <- function(expression_pc, loading, factor_number) {
	df <- data.frame(expression_pc=expression_pc, loading=loading)
	plotter <- ggplot(df, aes(x=expression_pc, y=loading)) + 
	           geom_point(size=.1) +
	           gtex_v8_figure_theme() + 
	           labs(x="Expression PC1", y = paste0("Loading ", factor_number)) + 
	           theme(legend.text = element_text(size=8), legend.title = element_text(size=8)) +
	           geom_smooth()
	return(plotter)
...
loading_expression_pc1_scatter_with_row_for_every_factor <- function(expression_pc, loadings) {
	factor_number <- 1
	factor_1_scatterplot <- make_loading_expression_pc_scatter_for_each_factor(expression_pc, loadings[, factor_number], factor_number)


	factor_number <- 2
	factor_2_scatterplot <- make_loading_expression_pc_scatter_for_each_factor(expression_pc, loadings[, factor_number], factor_number)
...
make_covariate_loading_correlation_heatmap <- function(covariates, loadings) {
    ...
    num_covs = dim(covariates)[2]
    valid_covariates <- c()
    for (cov_num in 1:num_covs) {
    	num_levels = length(unique(covariates[, cov_num]))
    	if (num_levels > 1) {
    		if (is.factor(covariates[, cov_num]) && num_levels < 100) {
    			valid_covariates <- c(valid_covariates, cov_num)
    		} else if (is.factor(covariates[, cov_num]) == FALSE) {
    			valid_covariates <- c(valid_covariates, cov_num)
    		}
    	}
    }
...
umap_scatter <- make_umap_loading_scatter_plot_colored_by_real_valued_variable(covariates$time, umap_loadings, "Day")
ggsave(umap_scatter, file=output_file, width=7.2, height=6.0, units="in")
...
umap_scatter <- make_umap_loading_scatter_plot_colored_by_real_valued_variable(covariates$expression_pc1, umap_loadings, "expression pc1")
ggsave(umap_scatter, file=output_file, width=7.2, height=6.0, units="in")
...
umap_scatter <- make_umap_loading_scatter_plot_colored_by_categorical_variable(covariates$cell_line, umap_loadings, "cell_line")
ggsave(umap_scatter, file=output_file, width=7.2, height=6.0, units="in")
...
scatterplot <- loading_expression_pc1_scatter_with_row_for_every_factor(covariates$expression_pc1, loadings)
...
boxplot <- make_loading_boxplot_plot_with_row_for_every_factor_by_categorical_covariate(factor(covariates$time), covariates$time, loadings, "Day")
