...
import ase_factorization_with_covariates
...
def train_ase_factorization_model(ase_file, covariate_file, k, output_dir):
	allelic_counts, total_counts = load_in_ase_data(ase_file)
	cov = np.loadtxt(covariate_file)
	ase_factorization_obj = ase_factorization_with_covariates.ASE_FACTORIZATION(K=k, output_root=output_dir + 'ase_factorization')
...
train_ase_factorization_model(ase_file, covariate_file, k,  output_dir)