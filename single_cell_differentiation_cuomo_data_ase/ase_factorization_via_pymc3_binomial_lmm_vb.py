...
class ASE_FACTORIZATION(object):
	...
	def fit(self, allelic_counts, total_counts, cov, z_sample):
		""" Fit the model.
			Args:
		"""
	...
	def run_factorization(self):
		...
		with pm.Model() as bb_glm:
			...
			p_mat = pm.math.invlogit(pm.math.dot(self.cov, BETA.T) + pm.math.dot(U,V.T) + A_SAMPLE[self.Z_sample,:])
			#conc_mat = pm.math.dot(np.ones((self.N,1)), CONC)
			R = pm.Binomial('like', p=p_mat[~nans], n=self.total_counts[~nans], observed=self.allelic_counts[~nans])
			approx = pm.fit(method='advi', n=200000)
