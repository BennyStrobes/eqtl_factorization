...
def outside_update_V_t(U_new, ys, ns, phi, z, concShape, concRate, cauchy_scale):
	########################################
	# load in your data
	########################################
...
class ASE_FACTORIZATION(object):
		...
		for sample_iter in range(self.N):
			...
			try:
				...
				try:
					...
					try:
						op = BB_FIXED_CONC_GLM.optimizing(data = data, verbose=False,iter=5000,seed=1, algorithm="Newton")
					except RuntimeError:
						try:
							op = BB_FIXED_CONC_GLM.optimizing(data = data, verbose=False,iter=5000,seed=2, algorithm="Newton")
						except RuntimeError:
							print('eror')
							pdb.set_trace()
	...
	def update_V_and_conc(self):
		...
		if parrallel == False:
			...
			for test_index in range(self.T):
				print(test_index)
				V_update_data.append(outside_update_V_t(U_new, self.allelic_counts[:, test_index], self.total_counts[:, test_index], self.phi[:, test_index, :], self.z, self.concShape, self.concRate, self.cauchy_scale))
		elif parrallel == True:
			U_new = np.hstack((self.cov, self.U))
			V_update_data = Parallel(n_jobs=24)(delayed(outside_update_V_t)(U_new, self.allelic_counts[:, test_index], self.total_counts[:, test_index], self.phi[:, test_index, :], self.z, self.concShape, self.concRate, self.cauchy_scale) for test_index in range(self.T))
