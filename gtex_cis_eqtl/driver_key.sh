...
file_stem="eqtl_factorization_"$num_latent_factors"_factors"



################################
# Run eqtl factorization over a number of parameters
#genotype_intercepts=( "True_penalized" "True" "False")
#lasso_param_us=( "0.0001" "0.001" "0.01" "0.1" "1")
#asso_param_vs=( "0.0" "0.0001" "0.001" "0.01" "0.1" "1")
genotype_intercepts=("False")
initializations=("random")
lasso_param_us=("0.0001")
lasso_param_vs=("0.0001")
################################
# Loop through covariate methods
for genotype_intercept in "${genotype_intercepts[@]}"; do
	for lasso_param_u in "${lasso_param_us[@]}"; do
		for lasso_param_v in "${lasso_param_vs[@]}"; do
			for initialization in "${initializations[@]}"; do
				sh eqtl_factorization.sh $sample_overlap_file $expression_training_file $genotype_training_file $expression_testing_file $genotype_testing_file $num_latent_factors $file_stem $eqtl_results_dir $lasso_param_u $lasso_param_v $genotype_intercept $initialization 
			done
		done
	done
done
