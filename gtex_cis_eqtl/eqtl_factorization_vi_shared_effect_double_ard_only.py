...
def compute_kl_divergence_of_gaussian_bernoulli(W_mu, W_var, gamma_alpha, gamma_beta, gamma_component_alpha, gamma_component_beta, K):
	...
	log_component_gamma_expected = special.digamma(gamma_component_alpha) - np.log(gamma_component_beta)
	gamma_component_expected = gamma_component_alpha/gamma_component_beta
	...
	for k in range(K):
		likelihood_term_a = likelihood_term_a + np.sum((1/2.0)*log_component_gamma_expected[k]*log_gamma_expected[k,:])
		likelihood_term_b = likelihood_term_b - (np.sum(W_squared_expected_val[k,:]*gamma_component_expected[k]*gamma_expected[k, :]/2.0))
...
class EQTL_FACTORIZATION_VI(object):
	...
	def fit(self, G, Y, z):
		...
		for vi_iter in range(self.max_iter):
			...
			print('##############')
			print(self.gamma_component_U_alpha/self.gamma_component_U_beta)
			print(self.gamma_U_alpha[0:2,:]/self.gamma_U_beta[0:2,:])
			#print(self.gamma_component_V_alpha/self.gamma_component_V_beta)
			#print(self.gamma_component_F_alpha/self.gamma_component_F_beta)
			print('##############')
	...
	def update_U_nk(self, sample_index, k):
		...
		gamma_k_u_expected_val = (self.gamma_U_alpha[sample_index, k]/self.gamma_U_beta[sample_index, k])*(self.gamma_component_U_alpha[k]/self.gamma_component_U_beta[k])
	...
	def update_V_kt(self, k, test_index):
		# Compute relevent expectations
		gamma_k_v_expected_val = (self.gamma_V_alpha[k, test_index]/self.gamma_V_beta[k, test_index])*(self.gamma_component_V_alpha[k]/self.gamma_component_V_beta[k])
	...
	def update_F_t(self, test_index):
		# Compute relevent expectations
		gamma_f_expected_val = (self.gamma_F_alpha[test_index]/self.gamma_F_beta[test_index])*(self.gamma_component_F_alpha/self.gamma_component_F_beta)
		tau_t_expected_val = self.tau_alpha[test_index]/self.tau_beta[test_index]
		alpha_t_expected_val = self.alpha_big_mu[:,test_index]
		U_S_expected_val = self.U_mu
		V_S_t_expected_val = self.V_mu[:,test_index]
	...
	def update_gamma_V(self):
		gamma_component_mean = self.gamma_component_V_alpha/self.gamma_component_V_beta
		V_squared_expected_val = ((np.square(self.V_mu) + self.V_var))
		self.gamma_V_alpha = np.ones((self.K, self.T))*(self.alpha_prior + (1/2.0))
		for k in range(self.K):
			self.gamma_V_beta[k,:] = self.beta_prior + ((gamma_component_mean[k]*V_squared_expected_val[k,:])/2.0)
			gamma_element_k_mean = self.gamma_V_alpha[k,:]/self.gamma_V_beta[k,:]
			# update components
			self.gamma_component_V_alpha[k] = self.alpha_prior + (self.T/2.0)
			self.gamma_component_V_beta[k] = self.beta_prior + np.sum(gamma_element_k_mean*V_squared_expected_val[k,:])/2.0
	def update_gamma_U(self):
		# Update component level 
		U_squared_expected_val = ((np.square(self.U_mu) + self.U_var))
		#for k in range(self.K):
			#gamma_element_k_mean = self.gamma_U_alpha[:, k]/self.gamma_U_beta[:, k]
			#self.gamma_component_U_alpha[k] = self.alpha_prior + (self.N/2.0)
			#self.gamma_component_U_beta[k] = self.beta_prior + np.sum(gamma_element_k_mean*U_squared_expected_val[:,k])/2.0
		gamma_component_mean = self.gamma_component_U_alpha/self.gamma_component_U_beta
		self.gamma_U_alpha = np.ones((self.N, self.K))*(self.alpha_prior + (1/2.0))
		for k in range(self.K):
			self.gamma_U_beta[:, k] = self.beta_prior + ((gamma_component_mean[k]*U_squared_expected_val[:,k])/2.0)
	def update_gamma_F(self):
		F_squared_expected_val = ((np.square(self.F_mu) + self.F_var))
		gamma_component_mean = self.gamma_component_F_alpha/self.gamma_component_F_beta
		self.gamma_F_alpha = np.ones(self.T)*(self.alpha_prior + (1.0/2.0))
		self.gamma_F_beta = self.beta_prior + ((gamma_component_mean*F_squared_expected_val)/2.0)
		gamma_element_k_mean = self.gamma_F_alpha/self.gamma_F_beta
		self.gamma_component_F_alpha = self.alpha_prior + (self.T/2.0)
		self.gamma_component_F_beta = self.beta_prior + np.sum(gamma_element_k_mean*F_squared_expected_val)/2.0
	...
	def compute_kl_divergence_of_gamma_f(self):
		...
		gamma_alpha = np.asarray([self.gamma_component_F_alpha])
		gamma_beta = np.asarray([self.gamma_component_F_beta])
		kl_divergence = kl_divergence + compute_kl_divergence_of_gamma(alpha_prior, beta_prior, gamma_alpha, gamma_beta)
	...
	def compute_kl_divergence_of_gamma_u(self):
		...
		gamma_alpha = self.gamma_component_U_alpha
		gamma_beta = self.gamma_component_V_beta
		kl_divergence = kl_divergence + compute_kl_divergence_of_gamma(alpha_prior, beta_prior, gamma_alpha, gamma_beta)
	...
	def compute_kl_divergence_of_gamma_v(self):
		...
		gamma_alpha = self.gamma_component_V_alpha
		gamma_beta = self.gamma_component_V_beta
		kl_divergence = kl_divergence + compute_kl_divergence_of_gamma(alpha_prior, beta_prior, gamma_alpha, gamma_beta)
		return kl_divergence
	...
	def compute_kl_divergence_of_F_S(self):
		...
		gamma_alpha = np.asarray([self.gamma_F_alpha])
		gamma_beta = np.asarray([self.gamma_F_beta])
		gamma_component_alpha = np.asarray([self.gamma_component_F_alpha])
		gamma_component_beta = np.asarray([self.gamma_component_F_beta])
		kl_divergence = compute_kl_divergence_of_gaussian_bernoulli(W_mu, W_var, gamma_alpha, gamma_beta, gamma_component_alpha, gamma_component_beta, 1)
		return kl_divergence
	...
	def compute_kl_divergence_of_V_S(self):
		...
		gamma_alpha = self.gamma_V_alpha
		gamma_beta = self.gamma_V_beta
		gamma_component_alpha = self.gamma_component_V_alpha
		gamma_component_beta = self.gamma_component_V_beta
		kl_divergence = compute_kl_divergence_of_gaussian_bernoulli(W_mu, W_var, gamma_alpha, gamma_beta, gamma_component_alpha, gamma_component_beta, self.K)
		return kl_divergence
	...
	def compute_kl_divergence_of_U_S(self):
		...
		gamma_alpha = np.transpose(self.gamma_U_alpha)
		gamma_beta = np.transpose(self.gamma_U_beta)
		gamma_component_alpha = self.gamma_component_U_alpha
		gamma_component_beta = self.gamma_component_U_beta
		kl_divergence = compute_kl_divergence_of_gaussian_bernoulli(W_mu, W_var, gamma_alpha, gamma_beta, gamma_component_alpha, gamma_component_beta, self.K)
		return kl_divergence
	...
	def initialize_variables(self):
		...
		self.gamma_U_beta = 10*np.ones((self.N, self.K))
		...
		# precisions
		self.gamma_component_U_alpha = np.ones(self.K)
		self.gamma_component_U_beta = 10*np.ones(self.K)
		self.gamma_component_V_alpha = np.ones(self.K)
		self.gamma_component_V_beta = np.ones(self.K)
		self.gamma_component_F_alpha = 1
		self.gamma_component_F_beta = 1.0
