...
def outside_update_V_t(V_mu, V_var, S_V, V_var_s_0, G_slice, Y_slice, K, U_S_expected_val, U_S_squared_expected_val, F_S_t_expected_val, intercept_mu, S_slice, gamma_v, tau_t_expected_val, cov, C_mu_t, alpha_t_mu,theta_V_a, theta_V_b, sample_batch_fraction, step_size, SVI):
	...
	def fit(self, G, Y, cov, z):
		...
		for vi_iter in range(self.max_iter):
			...
			if vi_iter == 5: 
				...
				np.savetxt(self.output_root + '_iter_5_temper_V.txt', (self.V_mu*self.S_V), fmt="%s", delimiter='\t')
				...
				np.savetxt(self.output_root + '_iter_5_temper_theta_V.txt', self.theta_V_a/(self.theta_V_a + self.theta_V_b), fmt="%s", delimiter='\t')
