...
def outside_update_cov_n(cov_mu, cov_var, G_slice, Y_slice, J, U_S_slice,  V_S_expected_val, F_mu, intercept_mu, tau_expected_val, C_expected_val, C_squared_expected_val, alpha_i_expected_val, Delta_mu):
	component_predictions = U_S_slice@V_S_expected_val
	non_genetic_component_predictions = U_S_slice@Delta_mu
	for k in range(J):
		# Compute relevent expectations
		cov_mu_expected_val = cov_mu
		
		# Compute expectations on other components
		other_covariates_expected = (cov_mu_expected_val@C_expected_val) - cov_mu_expected_val[k]*C_expected_val[k,:]
		# Update variance of q(U|s=1)
		a_term = np.sum(tau_expected_val*C_squared_expected_val[k,:]) 
		cov_var[k] = 1.0/a_term

		# Update mean of q(U|s=1)
		resid = Y_slice - intercept_mu - alpha_i_expected_val - (other_covariates_expected) - non_genetic_component_predictions - G_slice*(F_mu + component_predictions)
		b_term = np.sum(tau_expected_val*C_expected_val[k,:]*resid)
		cov_mu[k] = cov_var[k]*b_term
...
def outside_update_C_t(C_mu, C_var, G_slice, Y_slice, K, U_S_expected_val, V_S_t_expected_val, F_S_t_expected_val, intercept_mu, S_slice, gamma_v, tau_t_expected_val, cov, squared_cov, alpha_t_mu, delta_t_mu, sample_batch_fraction, step_size, SVI):
	num_cov = cov.shape[1]
	...
	for j in range(num_cov):
		...
		# Update variance of q(V|s=1)
		a_term = (1.0/sample_batch_fraction)*(tau_t_expected_val*np.sum(S_slice*(squared_cov[:, j])))
...
class EQTL_FACTORIZATION_VI(object):
	def __init__(self, K=25,J=20, alpha=1e-3, beta=1e-3, a=1, b=1, gamma_v=1.0, gamma_u=1.0, max_iter=1000, delta_elbo_threshold=1e-8, SVI=False, parrallel_boolean=False, sample_batch_fraction=.3, learning_rate=.4, forgetting_rate=.01, num_test_cores=24, num_sample_cores=24, output_root=''):
	...
	def fit(self, G, Y, cov, z):
		...
		for vi_iter in range(self.max_iter):
			...
			if np.mod(vi_iter, 5) == 0: 
				...
				np.savetxt(self.output_root + '_temper_cov.txt', (self.cov_mu), fmt="%s", delimiter='\t')
	...
	def update_C(self):
		...
		cov_expected_val = self.cov_mu
		squared_cov_expected_val = np.square(self.cov_mu) + self.cov_var
	...
	def update_cov(self):
		C_squared_expected_val = np.square(self.C_mu) + self.C_var
		C_expected_val = self.C_mu
		...
		if self.parrallel_boolean == False:
			for sample_index in range(self.N):
				cov_update_data.append(outside_update_cov_n(cov_mu_copy[sample_index,:], cov_var_copy[sample_index,:], self.G[sample_index, :], self.Y[sample_index, :],self.J, U_S_expected_val[sample_index,:],  V_S_expected_val, self.F_mu, self.intercept_mu, self.tau_alpha/self.tau_beta, C_expected_val, C_squared_expected_val, self.alpha_big_mu[sample_index,:], self.Delta_mu))
		# Parrallelize
		elif self.parrallel_boolean == True:
			cov_update_data = Parallel(n_jobs=self.num_sample_cores)(delayed(outside_update_cov_n)(cov_mu_copy[sample_index,:], cov_var_copy[sample_index,:], self.G[sample_index, :], self.Y[sample_index, :],self.J, U_S_expected_val[sample_index,:],  V_S_expected_val, self.F_mu, self.intercept_mu, self.tau_alpha/self.tau_beta, C_expected_val, C_squared_expected_val, self.alpha_big_mu[sample_index,:], self.Delta_mu) for sample_index in range(self.N))

		# Convert to array
		cov_update_data = np.asarray(cov_update_data)
		# Fill in data structures
		self.cov_mu = cov_update_data[:, (self.J*0):(1*self.J)]
		self.cov_var = cov_update_data[:, (self.J*1):(2*self.J)]
	...
	def update_U(self):
		...
		if self.parrallel_boolean == False:
			for sample_index in range(self.N):
				U_update_data.append(outside_update_U_n(U_mu_copy[sample_index,:],S_U_copy[sample_index,:], U_var_copy[sample_index,:],U_var_s_0_copy[sample_index,:], self.G[sample_index, :], self.Y[sample_index, :], self.K, V_S_expected_val, V_S_squared_expected_val, self.F_mu, self.S[sample_index, :], self.intercept_mu, self.gamma_u, self.tau_alpha/self.tau_beta, self.cov_mu[sample_index,:], self.C_mu, self.alpha_big_mu[sample_index,:], self.theta_U_a, self.theta_U_b, self.Delta_mu, Delta_squared_expected_val))
		# Parrallelize
		elif self.parrallel_boolean == True:
			U_update_data = Parallel(n_jobs=self.num_sample_cores)(delayed(outside_update_U_n)(U_mu_copy[sample_index,:],S_U_copy[sample_index,:], U_var_copy[sample_index,:],U_var_s_0_copy[sample_index,:], self.G[sample_index, :], self.Y[sample_index, :], self.K, V_S_expected_val, V_S_squared_expected_val, self.F_mu, self.S[sample_index, :], self.intercept_mu, self.gamma_u, self.tau_alpha/self.tau_beta, self.cov_mu[sample_index,:], self.C_mu, self.alpha_big_mu[sample_index,:], self.theta_U_a, self.theta_U_b, self.Delta_mu, Delta_squared_expected_val) for sample_index in range(self.N))
	...
	def update_intercept(self):
		...
		if self.parrallel_boolean == False:
			for test_index in range(self.T):
				intercept_update_data.append(outside_update_intercept_t(intercept_mu_copy[test_index], intercept_var_copy[test_index], self.G[:, test_index], self.Y[:, test_index], self.N, U_S_expected_val, V_S_expected_val[:,test_index], self.F_mu[test_index], tau_expected_val[test_index], self.S[:, test_index], self.cov_mu, self.C_mu[:, test_index], self.alpha_big_mu[:, test_index], self.Delta_mu[:, test_index], self.sample_batch_fraction, self.step_size, self.SVI))
		else:
				intercept_update_data = Parallel(n_jobs=self.num_test_cores)(delayed(outside_update_intercept_t)(intercept_mu_copy[test_index], intercept_var_copy[test_index], self.G[:, test_index], self.Y[:, test_index], self.N, U_S_expected_val, V_S_expected_val[:,test_index], self.F_mu[test_index], tau_expected_val[test_index], self.S[:, test_index], self.cov_mu, self.C_mu[:, test_index], self.alpha_big_mu[:, test_index], self.Delta_mu[:, test_index], self.sample_batch_fraction, self.step_size, self.SVI) for test_index in range(self.T))
	...
	def update_tau(self):
		...
		if self.parrallel_boolean == False:
			for test_index in range(self.T):
				tau_update_data.append(outside_update_tau_t(tau_alpha_copy[test_index], tau_beta_copy[test_index], self.G[:, test_index], self.Y[:, test_index], self.N, U_S, V_S[:,test_index], self.F_mu[test_index], self.intercept_mu[test_index], V_S_squared[:, test_index], F_S_squared[test_index], U_S_squared, intercept_squared[test_index], self.S[:, test_index], self.alpha_prior, self.beta_prior, self.cov_mu, cov_squared, self.C_mu[:, test_index], C_squared[:, test_index], self.alpha_big_mu[:, test_index], self.alpha_big_var[:, test_index], self.Delta_mu[:, test_index], Delta_squared[:, test_index], self.sample_batch_fraction, self.step_size, self.SVI))
		else:
			tau_update_data = Parallel(n_jobs=self.num_test_cores)(delayed(outside_update_tau_t)(tau_alpha_copy[test_index], tau_beta_copy[test_index], self.G[:, test_index], self.Y[:, test_index], self.N, U_S, V_S[:,test_index], self.F_mu[test_index], self.intercept_mu[test_index], V_S_squared[:, test_index], F_S_squared[test_index], U_S_squared, intercept_squared[test_index], self.S[:, test_index], self.alpha_prior, self.beta_prior, self.cov_mu, cov_squared, self.C_mu[:, test_index], C_squared[:, test_index], self.alpha_big_mu[:, test_index], self.alpha_big_var[:, test_index], self.Delta_mu[:, test_index], Delta_squared[:, test_index], self.sample_batch_fraction, self.step_size, self.SVI) for test_index in range(self.T))
	...
	def initialize_variables(self):
		...
		if self.SVI == False:
			...
			self.cov_known = np.copy(self.cov_full_known)
			...
			pca = sklearn.decomposition.PCA(n_components=self.J, whiten=True)
			pca.fit(np.random.randn(self.N, 9999).T)
			self.cov_mu = pca.components_.T
			for j in range(self.J):
				self.cov_mu[:,j] = ((self.cov_mu[:,j]-np.mean(self.cov_mu[:,j]))/(np.std(self.cov_mu[:,j])))
			self.cov_var = np.ones((self.N, self.J))*100.0
			self.cov_mu[:, :(self.cov_known.shape[1])] = np.copy(self.cov_known)
		...
		V_init, betas, C_init, intercepts, delta_init = run_linear_model_for_initialization(self.Y, self.G, self.cov_mu, self.U_mu)
