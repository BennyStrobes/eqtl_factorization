...
def run_linear_model_for_initialization(Y, G, cov, U):
	...
	for test_number in range(num_tests):
		...
		model_covariates = np.hstack((np.transpose(np.asmatrix(g_vec)), cov, U_scaled))
...
def outside_update_U_n(U_mu, U_var, G_slice, Y_slice, K, V_S_expected_val, V_S_squared_expected_val, F_S_expected_val, S_slice, intercept_mu, gamma_u, tau_expected_val, cov_slice, C_mu, alpha_i_expected_val):
...
def outside_update_V_t(V_mu, V_var, G_slice, Y_slice, K, U_S_expected_val, U_S_squared_expected_val, F_S_t_expected_val, intercept_mu, S_slice, gamma_v, tau_t_expected_val, cov, C_mu_t, alpha_t_mu, sample_batch_fraction, step_size, SVI):
...
def outside_update_C_t(C_mu, C_var, G_slice, Y_slice, K, U_S_expected_val, V_S_t_expected_val, F_S_t_expected_val, intercept_mu, S_slice, gamma_v, tau_t_expected_val, cov, alpha_t_mu, sample_batch_fraction, step_size, SVI):
	num_cov = cov.shape[1]
	...
	for j in range(num_cov):
		...
		resid = Y_slice - intercept_mu - alpha_t_mu - other_covariates_expected - G_slice*(components_expected + F_S_t_expected_val)
...
def outside_update_intercept_t(intercept_mu, intercept_var, G_slice, Y_slice, N, U_S_expected_val, V_S_t_expected_val, F_S_t_expected_val, tau_t_expected_val, S_slice, cov, C_mu_t, alpha_t_mu, sample_batch_fraction, step_size, SVI):
	# Compute relevent expectations
	# Compute expectations on other components
	components_expected = U_S_expected_val@V_S_t_expected_val
	covariates_expected = cov@C_mu_t
	resid = (S_slice*Y_slice) - (S_slice*alpha_t_mu) - (S_slice*G_slice*(F_S_t_expected_val + components_expected)) - (S_slice*covariates_expected)
...
def outside_update_F_t(F_mu, F_var, G_slice, Y_slice, U_S_expected_val, V_S_t_expected_val, intercept_mu_t, gamma_f_expected_val, tau_t_expected_val, S_slice, cov, C_mu_t, alpha_t_mu, sample_batch_fraction, step_size, SVI):
	# Compute expectations on other components
	components_expected = U_S_expected_val@V_S_t_expected_val
	covariates_expected = cov@C_mu_t
...
def outside_add_tau_t(G_slice, Y_slice, N, U_S, V_S_t, F_S_t, intercept_t, V_S_t_squared, F_S_t_squared, U_S_squared, intercept_t_squared, alpha_mu_t, alpha_var_t, cov, cov_squared, C_t, C_squared_t, alpha_prior, beta_prior, sample_batch_fraction, step_size, SVI):
	# Compute Relevent expectations
	...
	resid = np.square(Y_slice) + alpha_t_squared + intercept_t_squared + squared_covariate_terms + np.square(G_slice)*(F_S_t_squared + squared_factor_terms)
	# Now add terms with Y
	resid = resid - (2.0*Y_slice*(covariate_terms + intercept_t + alpha_mu_t + G_slice*factor_terms + G_slice*F_S_t))

	resid = resid + 2.0*covariate_terms*(intercept_t + alpha_mu_t + G_slice*(factor_terms + F_S_t))
...
class EQTL_FACTORIZATION_VI(object):
	def __init__(self, K=25, alpha=1e-3, beta=1e-3, a=1, b=1, gamma_v=1.0, gamma_u=1.0, max_iter=1000, delta_elbo_threshold=1e-8, SVI=False, parrallel_boolean=False, sample_batch_fraction=.3, learning_rate=.4, forgetting_rate=.01, num_test_cores=24, num_sample_cores=24, output_root=''):
	...
	def fit(self, G, Y, cov, z):
		...
		for vi_iter in range(self.max_iter):
			...
			print('day0')
			print(self.U_mu[790:795,:])
			print('day1')
			print(self.U_mu[0:5,:])
			print('day2')
			print(self.U_mu[1710:1715,:])
			print('day3')
			print(self.U_mu[1761:1765,:])
			...
			if vi_iter == 5: 
				print(vi_iter)
				np.savetxt(self.output_root + '_iter_5_temper_U_S.txt', (self.U_mu), fmt="%s", delimiter='\t')
				#np.savetxt(self.output_root + '_temper2_S.txt', (self.S), fmt="%s", delimiter='\t')
				np.savetxt(self.output_root + '_iter_5_temper_V.txt', (self.V_mu), fmt="%s", delimiter='\t')
				np.savetxt(self.output_root + '_iter_5_temper_F.txt', (self.F_mu), fmt="%s", delimiter='\t')
				np.savetxt(self.output_root + '_iter_5_temper_intercept.txt', (self.intercept_mu), fmt="%s", delimiter='\t')
				np.savetxt(self.output_root + '_iter_5_temper_tau.txt', np.asmatrix(self.tau_alpha/self.tau_beta), fmt="%s", delimiter='\t')
				np.savetxt(self.output_root + '_iter_5_temper_C.txt', (self.C_mu), fmt="%s", delimiter='\t')
				np.savetxt(self.output_root + '_iter_5_temper_alpha.txt', (self.alpha_mu), fmt="%s", delimiter='\t')
				np.savetxt(self.output_root + '_iter_5_temper_psi.txt', self.psi_alpha/self.psi_beta, fmt="%s", delimiter='\t')
			if np.mod(vi_iter, 50) == 0 and vi_iter > 0:
				...
				print(vi_iter)
				np.savetxt(self.output_root + '_temper_U_S.txt', (self.U_mu), fmt="%s", delimiter='\t')
				#np.savetxt(self.output_root + '_temper2_S.txt', (self.S), fmt="%s", delimiter='\t')
				...
				np.savetxt(self.output_root + '_temper_tau.txt', np.asmatrix(self.tau_alpha/self.tau_beta), fmt="%s", delimiter='\t')
	...
	def update_alpha(self):
		...
		if self.parrallel_boolean == False:
			for test_index in range(self.T):
				alpha_update_data.append(outside_update_alpha_t(alpha_mu_copy[:, test_index], alpha_var_copy[:, test_index], self.G[:, test_index], self.Y[:, test_index], self.I, U_S_expected_val, self.V_mu[:, test_index], self.F_mu[test_index], self.intercept_mu[test_index], self.cov, self.C_mu[:, test_index], tau_expected_val, psi_expected_val[test_index], self.individual_to_sample_indices, self.individual_to_number_full_indices, self.step_size, self.SVI))
		else:
			alpha_update_data = Parallel(n_jobs=self.num_test_cores)(delayed(outside_update_alpha_t)(alpha_mu_copy[:, test_index], alpha_var_copy[:, test_index], self.G[:, test_index], self.Y[:, test_index], self.I, U_S_expected_val, self.V_mu[:, test_index], self.F_mu[test_index], self.intercept_mu[test_index], self.cov, self.C_mu[:, test_index], tau_expected_val, psi_expected_val[test_index], self.individual_to_sample_indices, self.individual_to_number_full_indices, self.step_size, self.SVI) for test_index in range(self.T))
	...
	def update_U(self):
		...
		if self.parrallel_boolean == False:
			for sample_index in range(self.N):
				U_update_data.append(outside_update_U_n(U_mu_copy[sample_index,:], U_var_copy[sample_index,:], self.G[sample_index, :], self.Y[sample_index, :], self.K, self.V_mu, V_S_squared_expected_val, self.F_mu, self.S[sample_index, :], self.intercept_mu, self.gamma_u, self.tau_alpha/self.tau_beta, self.cov[sample_index,:], self.C_mu, self.alpha_big_mu[sample_index,:]))
		# Parrallelize
		elif self.parrallel_boolean == True:
			U_update_data = Parallel(n_jobs=self.num_sample_cores)(delayed(outside_update_U_n)(U_mu_copy[sample_index,:], U_var_copy[sample_index,:], self.G[sample_index, :], self.Y[sample_index, :], self.K, self.V_mu, V_S_squared_expected_val, self.F_mu, self.S[sample_index,:], self.intercept_mu, self.gamma_u, self.tau_alpha/self.tau_beta, self.cov[sample_index,:], self.C_mu, self.alpha_big_mu[sample_index,:]) for sample_index in range(self.N))
	...
	def update_intercept(self):
		...
		if self.parrallel_boolean == False:
			for test_index in range(self.T):
				intercept_update_data.append(outside_update_intercept_t(intercept_mu_copy[test_index], intercept_var_copy[test_index], self.G[:, test_index], self.Y[:, test_index], self.N, U_S_expected_val, self.V_mu[:,test_index], self.F_mu[test_index], tau_expected_val, self.S[:, test_index], self.cov, self.C_mu[:, test_index], self.alpha_big_mu[:, test_index], self.sample_batch_fraction, self.step_size, self.SVI))
		if self.parrallel_boolean == True:
				intercept_update_data = Parallel(n_jobs=self.num_test_cores)(delayed(outside_update_intercept_t)(intercept_mu_copy[test_index], intercept_var_copy[test_index], self.G[:, test_index], self.Y[:, test_index], self.N, U_S_expected_val, self.V_mu[:,test_index], self.F_mu[test_index], tau_expected_val, self.S[:, test_index], self.cov, self.C_mu[:, test_index], self.alpha_big_mu[:, test_index], self.sample_batch_fraction, self.step_size, self.SVI) for test_index in range(self.T))
	...
	def initialize_variables(self):
		...
		V_init, betas, C_init, intercepts = run_linear_model_for_initialization(self.Y, self.G, self.cov, self.U_mu)
