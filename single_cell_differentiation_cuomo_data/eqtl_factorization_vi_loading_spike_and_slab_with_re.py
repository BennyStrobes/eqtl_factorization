...
def outside_update_U_n(U_mu, S_U, U_var,U_var_s_0, G_slice, Y_slice, K, V_S_expected_val, V_S_squared_expected_val, F_S_expected_val, S_slice, intercept_mu, gamma_u, tau_expected_val, cov_slice, C_mu, alpha_i_expected_val, theta_U_a, theta_U_b):
...
class EQTL_FACTORIZATION_VI(object):
	...
	def fit(self, G, Y, cov, z):
		...
		for vi_iter in range(self.max_iter):
			...
			if vi_iter == 5: 
				print(vi_iter)
				np.savetxt(self.output_root + '_iter_5_temper_U_S.txt', (self.U_mu*self.S_U), fmt="%s", delimiter='\t')
				np.savetxt(self.output_root + '_iter_5_temper_S_U.txt', (self.S_U), fmt="%s", delimiter='\t')
				...
				np.savetxt(self.output_root + '_iter_5_temper_tau.txt', (self.tau_alpha/self.tau_beta), fmt="%s", delimiter='\t')
				np.savetxt(self.output_root + '_iter_5_temper_theta_U.txt', self.theta_U_a/(self.theta_U_a + self.theta_U_b), fmt="%s", delimiter='\t')
	...
	def update_U(self):
		...
		if self.parrallel_boolean == False:
			for sample_index in range(self.N):
				U_update_data.append(outside_update_U_n(U_mu_copy[sample_index,:],S_U_copy[sample_index,:], U_var_copy[sample_index,:],U_var_s_0_copy[sample_index,:], self.G[sample_index, :], self.Y[sample_index, :], self.K, self.V_mu, V_S_squared_expected_val, self.F_mu, self.S[sample_index, :], self.intercept_mu, self.gamma_u, self.tau_alpha/self.tau_beta, self.cov[sample_index,:], self.C_mu, self.alpha_big_mu[sample_index,:], self.theta_U_a, self.theta_U_b))
		# Parrallelize
		elif self.parrallel_boolean == True:
	...
	def initialize_variables(self):
		...
		if self.SVI == False:
			...
			self.U_var = np.ones((self.N, self.K))*(1.0/self.gamma_u) 
