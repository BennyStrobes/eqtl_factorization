...
def main():
	...
	parser.add_argument("--ref", required=True, help="Reference genome file (fasta with index)")
	...
	parser.add_argument("--max_depth", required=False, type=int, default=100000, help="Maximum mpileup depth")
	...
	for line in stream_in:
		...
		for record in records:
			...
			if int(vcf_cols[1]) == pos:
				...
				ds_index = vcf_cols[8].split(":").index("DS")
				ds = vcf_cols[vcf_map[args.sample]].split(":")[ds_index]
				if round(float(ds)) == 1:
					if len(alleles) != 2:
