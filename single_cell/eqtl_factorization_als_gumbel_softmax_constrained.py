...
SIMPLEX_CONSTRAINED_LM = pickle.load(open('update_U_gumbel_softmax_constrained_als.pkl', 'rb'))
...
def outside_update_U_n(g_sample, y_sample, K, V, temperature):
	# Get V scaled by genotype for this sample
	V_scaled = np.transpose(V)*g_sample[:,None]

	data = dict(N=V_scaled.shape[0], K=V_scaled.shape[1], x=V_scaled, y=y_sample, tau=temperature)
...
class EQTL_FACTORIZATION_ALS_CONSTRAINED(object):
	def __init__(self, K=5,genotype_intercept=True,max_iter=1000, lasso_param_v=.1, parrallel_boolean=False, num_test_cores=24, num_sample_cores=24, temperature=.5):
	...
	def fit(self, G, Y, z):
		...
		for iter_num in range(self.max_iter):
			...
			if np.mod(iter_num, 50) == 0 and iter_num > 0:
				...
				np.savetxt('/work-zfs/abattle4/bstrober/single_cell_eqtl_factorization/temp_output/temper_U_als_constrained_' + str(self.lasso_param_v) + '_' + str(self.genotype_intercept) + '_' + str(iter_num) + '.txt', (self.U), fmt="%s", delimiter='\t')
				np.savetxt('/work-zfs/abattle4/bstrober/single_cell_eqtl_factorization/temp_output/temper_V_als_constrained_' + str(self.lasso_param_v) + '_' + str(self.genotype_intercept) + '_' + str(iter_num) + '.txt', (self.V), fmt="%s", delimiter='\t')
				np.savetxt('/work-zfs/abattle4/bstrober/single_cell_eqtl_factorization/temp_output/temper_intercept_als_constrained_' + str(self.lasso_param_v) + '_' + str(self.genotype_intercept) + '_' + str(iter_num) + '.txt', (self.intercept), fmt="%s", delimiter='\t')
	...
	def update_U(self):
		...
		if self.parrallel_boolean == False:
			for sample_index in range(self.N):
				U_update_data.append(outside_update_U_n(self.G[sample_index, :], Y_scaled[sample_index, :], self.K, temp_V, self.temperature))
		# Parrallelize
		elif self.parrallel_boolean == True:
			U_update_data = Parallel(n_jobs=self.num_sample_cores)(delayed(outside_update_U_n)(self.G[sample_index, :], Y_scaled[sample_index, :], self.K, temp_V, self.temperature) for sample_index in range(self.N))
