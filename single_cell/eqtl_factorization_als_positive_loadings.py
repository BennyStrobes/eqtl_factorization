	...
	def fit(self, G, Y, z):
		...
		for iter_num in range(self.max_iter):
			...
			if np.mod(iter_num, 50) == 0 and iter_num > 0:
				...
				np.savetxt('/work-zfs/abattle4/bstrober/single_cell_eqtl_factorization/temp_output/temper_U_als_positive_loadings_' + str(self.lasso_param_v) + '_' + str(self.genotype_intercept) + '_' + str(iter_num) + '.txt', (self.U), fmt="%s", delimiter='\t')
				np.savetxt('/work-zfs/abattle4/bstrober/single_cell_eqtl_factorization/temp_output/temper_V_als_positive_loadings_' + str(self.lasso_param_v) + '_' + str(self.genotype_intercept) + '_' + str(iter_num) + '.txt', (self.V), fmt="%s", delimiter='\t')
				np.savetxt('/work-zfs/abattle4/bstrober/single_cell_eqtl_factorization/temp_output/temper_intercept_als_positive_loadings_' + str(self.lasso_param_v) + '_' + str(self.genotype_intercept) + '_' + str(iter_num) + '.txt', (self.intercept), fmt="%s", delimiter='\t')
