		...
		try: 
			...
			try: 
				...
				try: 
					...
					print('Errorr')
	'''
	clf = linear_model.ElasticNet(alpha=lasso_param, l1_ratio=1.0, fit_intercept=False)
	...
	def fit(self, G, Y, z):
		...
		for iter_num in range(self.max_iter):
			...
			if np.mod(iter_num, 50) == 0 and iter_num > 0:
				...
				np.savetxt('/work-zfs/abattle4/bstrober/single_cell_eqtl_factorization/temp_output/temper_U_als_unconstrained_' + str(self.lasso_param_v) + '_' + str(self.genotype_intercept) + '_' + str(iter_num) + '.txt', (self.U), fmt="%s", delimiter='\t')
				np.savetxt('/work-zfs/abattle4/bstrober/single_cell_eqtl_factorization/temp_output/temper_V_als_unconstrained_' + str(self.lasso_param_v) + '_' + str(self.genotype_intercept) + '_' + str(iter_num) + '.txt', (self.V), fmt="%s", delimiter='\t')
				np.savetxt('/work-zfs/abattle4/bstrober/single_cell_eqtl_factorization/temp_output/temper_intercept_als_unconstrained_' + str(self.lasso_param_v) + '_' + str(self.genotype_intercept) + '_' + str(iter_num) + '.txt', (self.intercept), fmt="%s", delimiter='\t')
	...
	def update_U(self):
		...
		if self.parrallel_boolean == False:
			for sample_index in range(self.N):
				U_update_data.append(outside_update_U_n(self.G[sample_index, :], Y_scaled[sample_index, :], self.K, temp_V, self.lasso_param_v))
		# Parrallelize
		elif self.parrallel_boolean == True:
			U_update_data = Parallel(n_jobs=self.num_sample_cores)(delayed(outside_update_U_n)(self.G[sample_index, :], Y_scaled[sample_index, :], self.K, temp_V, self.lasso_param_v) for sample_index in range(self.N))
