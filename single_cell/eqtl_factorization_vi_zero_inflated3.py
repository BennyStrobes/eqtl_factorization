...
def run_linear_model_for_initialization(Y, G, cov, K):
	num_tests = Y.shape[1]
	betas = []
	c_vec = []
	intercepts = []
	...
	for test_number in range(num_tests):
		...
		U_scaled = np.zeros(U_init.shape)
		for k in range(K):
			U_scaled[:, k] = U_init[:,k]*g_vec
		model_covariates = np.hstack((np.transpose(np.asmatrix(g_vec)), U_scaled, cov))
		reg = LinearRegression().fit(model_covariates, np.transpose(np.asmatrix(y_vec)))
		betas.append(reg.coef_[0][0])
		V_vec.append(reg.coef_[0][1:(1+K)])
		c_vec.append(reg.coef_[0][(1+K):])
		intercepts.append(reg.intercept_[0])
	return np.asarray(betas), np.asarray(c_vec), np.asarray(V_vec), np.asarray(intercepts)
...
def outside_update_cov_n(cov_mu, cov_var, G_slice, Y_slice, K, V_S_expected_val, U_S_expected_val, F_S_expected_val, intercept_mu, S_slice, gamma_v, tau_expected_val, C_mu, C_squared, num_cov, sample_batch_fraction, step_size, SVI):
	components_expected = U_S_expected_val@V_S_expected_val
	for j in range(num_cov):
		# Compute expectations on other covariates
		other_covariates_expected = (cov_mu@C_mu) - cov_mu[j]*C_mu[j,:]
		a_term = np.sum(tau_expected_val*S_slice*C_squared[j,:]) 
		cov_var[j] = 1.0/a_term

		resid = Y_slice - intercept_mu - (other_covariates_expected) - G_slice*(F_S_expected_val + components_expected)
		b_term = np.sum(tau_expected_val*C_mu[j,:]*S_slice*resid)

		cov_mu[j] = cov_var[j]*b_term
	return np.hstack((cov_mu, cov_var))
...
def outside_update_C_t(C_mu, C_var, G_slice, Y_slice, K, U_S_expected_val, V_S_t_expected_val, F_S_t_expected_val, intercept_mu, S_slice, gamma_v, tau_t_expected_val, cov, cov_squared, sample_batch_fraction, step_size, SVI):
	num_cov = cov.shape[1]
	...
	for j in range(num_cov):
		...
		# Update variance of q(V|s=1)
		a_term = (1.0/sample_batch_fraction)*(tau_t_expected_val*np.sum(S_slice*cov_squared[:, j]))
...
class EQTL_FACTORIZATION_VI(object):
	...
	def fit(self, G, Y, cov, z):
		...
		for vi_iter in range(self.max_iter):
			...
			print(self.U_mu[0:15,:])
			# Remove irrelevent factors
			if np.mod(vi_iter, 40) == 0 and vi_iter > 0:
				# UPDATE remove irrelevent_factors TO BE IN TERMS OF *_FULL (ie re-learn theta_U on all data)
				#self.remove_irrelevent_factors()
				print(vi_iter)
				np.savetxt('/work-zfs/abattle4/bstrober/single_cell_eqtl_factorization/single_cell/eqtl_factorization_results/temper3_U_S.txt', (self.U_mu), fmt="%s", delimiter='\t')
				np.savetxt('/work-zfs/abattle4/bstrober/single_cell_eqtl_factorization/single_cell/eqtl_factorization_results/temper3_S.txt', (self.S), fmt="%s", delimiter='\t')
				np.savetxt('/work-zfs/abattle4/bstrober/single_cell_eqtl_factorization/single_cell/eqtl_factorization_results/temper3_V.txt', (self.V_mu), fmt="%s", delimiter='\t')
				np.savetxt('/work-zfs/abattle4/bstrober/single_cell_eqtl_factorization/single_cell/eqtl_factorization_results/temper3_F.txt', (self.F_mu), fmt="%s", delimiter='\t')
				np.savetxt('/work-zfs/abattle4/bstrober/single_cell_eqtl_factorization/single_cell/eqtl_factorization_results/temper3_intercept.txt', (self.intercept_mu), fmt="%s", delimiter='\t')
				np.savetxt('/work-zfs/abattle4/bstrober/single_cell_eqtl_factorization/single_cell/eqtl_factorization_results/temper3_tau.txt', (self.tau_alpha/self.tau_beta), fmt="%s", delimiter='\t')
				np.savetxt('/work-zfs/abattle4/bstrober/single_cell_eqtl_factorization/single_cell/eqtl_factorization_results/temper3_C.txt', (self.C_mu), fmt="%s", delimiter='\t')
				np.savetxt('/work-zfs/abattle4/bstrober/single_cell_eqtl_factorization/single_cell/eqtl_factorization_results/temper3_cov.txt', (self.cov_mu), fmt="%s", delimiter='\t')
	...
	def update_C(self):
		...
		cov_squared = np.square(self.cov_mu) + self.cov_var
	...
	def update_cov(self):
		...
		U_S_expected_val = self.U_mu
		tau_expected_val = self.tau_alpha/self.tau_beta
		cov_mu_copy = np.copy(self.cov_mu)
		cov_var_copy = np.copy(self.cov_var)
		gamma_v = self.gamma_v
		C_squared = np.square(self.C_mu) + self.C_var

		num_cov = self.C_mu.shape[0]
		# Keep track of variables
		cov_update_data = []

		if self.parrallel_boolean == False:
			for sample_index in range(self.N):
				cov_update_data.append(outside_update_cov_n(cov_mu_copy[sample_index,:], cov_var_copy[sample_index, :], self.G[sample_index, :], self.Y[sample_index, :], self.K, self.V_mu, U_S_expected_val[sample_index,:], self.F_mu, self.intercept_mu, self.S[sample_index, :], gamma_v, tau_expected_val, self.C_mu, C_squared, num_cov, self.sample_batch_fraction, self.step_size, self.SVI))
		elif self.parrallel_boolean == True:
			cov_update_data = Parallel(n_jobs=self.num_test_cores)(delayed(outside_update_cov_n)(cov_mu_copy[sample_index,:], cov_var_copy[sample_index, :], self.G[sample_index, :], self.Y[sample_index, :], self.K, self.V_mu, U_S_expected_val[sample_index,:], self.F_mu, self.intercept_mu, self.S[sample_index, :], gamma_v, tau_expected_val, self.C_mu, C_squared, num_cov, self.sample_batch_fraction, self.step_size, self.SVI) for sample_index in range(self.N))

		J = self.cov_mu.shape[1]
		# Convert to array
		cov_update_data = np.asarray(cov_update_data)

		# Fill in data structures
		self.cov_mu = cov_update_data[:,(J*0):(1*J)]
		self.cov_var = cov_update_data[:,(J*1):(2*J)]
	...
	def update_U(self):
		...
		if self.parrallel_boolean == False:
			for sample_index in range(self.N):
				U_update_data.append(outside_update_U_n(U_mu_copy[sample_index,:], U_var_copy[sample_index,:], self.G[sample_index, :], self.Y[sample_index, :], self.K, self.V_mu, V_S_squared_expected_val, self.F_mu, self.S[sample_index, :], self.intercept_mu, self.gamma_u, self.tau_alpha/self.tau_beta, self.cov_mu[sample_index,:], self.C_mu))
		# Parrallelize
		elif self.parrallel_boolean == True:
			U_update_data = Parallel(n_jobs=self.num_sample_cores)(delayed(outside_update_U_n)(U_mu_copy[sample_index,:], U_var_copy[sample_index,:], self.G[sample_index, :], self.Y[sample_index, :], self.K, self.V_mu, V_S_squared_expected_val, self.F_mu, self.S[sample_index,:], self.intercept_mu, self.gamma_u, self.tau_alpha/self.tau_beta, self.cov_mu[sample_index,:], self.C_mu) for sample_index in range(self.N))
	...
	def update_intercept(self):
		...
		if self.parrallel_boolean == False:
			for test_index in range(self.T):
				intercept_update_data.append(outside_update_intercept_t(intercept_mu_copy[test_index], intercept_var_copy[test_index], self.G[:, test_index], self.Y[:, test_index], self.N, U_S_expected_val, self.V_mu[:,test_index], self.F_mu[test_index], tau_expected_val[test_index], self.S[:, test_index], self.cov_mu, self.C_mu[:, test_index], self.sample_batch_fraction, self.step_size, self.SVI))
		if self.parrallel_boolean == True:
				intercept_update_data = Parallel(n_jobs=self.num_test_cores)(delayed(outside_update_intercept_t)(intercept_mu_copy[test_index], intercept_var_copy[test_index], self.G[:, test_index], self.Y[:, test_index], self.N, U_S_expected_val, self.V_mu[:,test_index], self.F_mu[test_index], tau_expected_val[test_index], self.S[:, test_index], self.cov_mu, self.C_mu[:, test_index], self.sample_batch_fraction, self.step_size, self.SVI) for test_index in range(self.T))
	...
	def update_tau(self):
		...
		if self.parrallel_boolean == False:
			for test_index in range(self.T):
				tau_update_data.append(outside_update_tau_t(tau_alpha_copy[test_index], tau_beta_copy[test_index], self.G[:, test_index], self.Y[:, test_index], self.N, U_S, self.V_mu[:,test_index], self.F_mu[test_index], self.intercept_mu[test_index], V_S_squared[:, test_index], F_S_squared[test_index], U_S_squared, intercept_squared[test_index], self.S[:, test_index], self.alpha_prior, self.beta_prior, self.cov_mu, cov_squared, self.C_mu[:, test_index], C_squared[:, test_index], self.sample_batch_fraction, self.step_size, self.SVI))
		if self.parrallel_boolean == True:
	...
	def initialize_variables(self):
		...
		self.num_cov = self.cov_init.shape[1]
		...
		if self.SVI == False:
			...
			for k in range(self.K):
				self.U_mu[:,k] = np.abs(((self.U_mu[:,k]-np.mean(self.U_mu[:,k]))/np.std(self.U_mu[:,k])))
		...
		#self.cov_mu = np.random.standard_normal(size=self.cov_init.shape)
		self.cov_mu = self.cov_init*10.0
		self.cov_var = np.ones(self.cov_init.shape)
		...
		betas, C_init, V_init, intercepts = run_linear_model_for_initialization(self.Y, self.G, self.cov_mu, self.K)

		self.V_mu = np.transpose(V_init)
		self.U_mu = np.copy(self.cov_mu[:,0:self.K])
