...
def run_linear_model_for_initialization(Y, G):
	...
	return np.asarray(betas), np.asarray(intercepts)
...
def outside_update_U_n(U_mu, U_var, G_slice, Y_slice, K, V_S_expected_val, V_S_squared_expected_val, F_S_expected_val, S_slice, intercept_mu, gamma_u, tau_expected_val):
	for k in range(K):
...
def outside_update_V_t(V_mu, V_var, G_slice, Y_slice, K, U_S_expected_val, U_S_squared_expected_val, F_S_t_expected_val, intercept_mu, S_slice, gamma_v, tau_t_expected_val, sample_batch_fraction, step_size, SVI):
	for k in range(K):
		# Compute expectations on other components
...
def outside_update_intercept_t(intercept_mu, intercept_var, G_slice, Y_slice, N, U_S_expected_val, V_S_t_expected_val, F_S_t_expected_val, tau_t_expected_val, S_slice, sample_batch_fraction, step_size, SVI):
	# Compute relevent expectations
	# Compute expectations on other components
	components_expected = U_S_expected_val@V_S_t_expected_val
	resid = (S_slice*Y_slice) - (S_slice*G_slice*(F_S_t_expected_val + components_expected))
...
def outside_update_F_t(F_mu, F_var, G_slice, Y_slice, U_S_expected_val, V_S_t_expected_val, intercept_mu_t, gamma_f_expected_val, tau_t_expected_val, S_slice, sample_batch_fraction, step_size, SVI):
	# Compute expectations on other components
	components_expected = U_S_expected_val@V_S_t_expected_val
	...
	resid = Y_slice - intercept_mu_t - G_slice*(components_expected)
...
def outside_update_tau_t(tau_alpha, tau_beta, G_slice, Y_slice, N, U_S, V_S_t, F_S_t, intercept_t, V_S_t_squared, F_S_t_squared, U_S_squared, intercept_t_squared, S_slice, alpha_prior, beta_prior, sample_batch_fraction, step_size, SVI):
	# Compute Relevent expectations
	squared_factor_terms = U_S_squared@V_S_t_squared
	factor_terms = U_S@V_S_t

	# First add together square terms
	resid = np.square(Y_slice) + S_slice*intercept_t_squared + S_slice*np.square(G_slice)*(F_S_t_squared + squared_factor_terms)
	# Now add terms with Y
	resid = resid - (2.0*Y_slice*S_slice*(intercept_t + G_slice*factor_terms + G_slice*F_S_t))

	resid = resid + 2.0*intercept_t*S_slice*(G_slice*(factor_terms + F_S_t))
...
def outside_update_S_t(S_copy_slice, G_slice, Y_slice, N, U_S, V_S_t, F_S_t, intercept_t, V_S_t_squared, F_S_t_squared, U_S_squared, intercept_t_squared, tau_t, theta_S_a_t, theta_S_b_t, sample_batch_fraction, step_size, SVI):
	# Compute Relevent expectations
	squared_factor_terms = U_S_squared@V_S_t_squared
	factor_terms = U_S@V_S_t
	...
	resid = intercept_t_squared + np.square(G_slice)*(F_S_t_squared + squared_factor_terms)
	...
	z_term = ln_theta_S_expected_val - ln_1_minus_theta_S_expected_val - (resid*tau_t/2.0)
...
class EQTL_FACTORIZATION_VI(object):
	...
	def update_U(self):
		...
		if self.parrallel_boolean == False:
			for sample_index in range(self.N):
				U_update_data.append(outside_update_U_n(U_mu_copy[sample_index,:], U_var_copy[sample_index,:], self.G[sample_index, :], self.Y[sample_index, :], self.K, self.V_mu, V_S_squared_expected_val, self.F_mu, self.S[sample_index, :], self.intercept_mu, self.gamma_u, self.tau_alpha/self.tau_beta))
		# Parrallelize
		elif self.parrallel_boolean == True:
			U_update_data = Parallel(n_jobs=self.num_sample_cores)(delayed(outside_update_U_n)(U_mu_copy[sample_index,:], U_var_copy[sample_index,:], self.G[sample_index, :], self.Y[sample_index, :], self.K, self.V_mu, V_S_squared_expected_val, self.F_mu, self.S[sample_index,:], self.intercept_mu, self.gamma_u, self.tau_alpha/self.tau_beta) for sample_index in range(self.N))
	...
	def update_F(self):
		...
		if self.parrallel_boolean == False:
			for test_index in range(self.T):
				F_update_data.append(outside_update_F_t(F_mu_copy[test_index], F_var_copy[test_index], self.G[:, test_index], self.Y[:, test_index], U_S_expected_val, self.V_mu[:,test_index], self.intercept_mu[test_index], self.gamma_v, tau_expected_val[test_index], self.S[:, test_index], self.sample_batch_fraction, self.step_size, self.SVI))
		if self.parrallel_boolean == True:
			F_update_data = Parallel(n_jobs=self.num_test_cores)(delayed(outside_update_F_t)(F_mu_copy[test_index], F_var_copy[test_index], self.G[:, test_index], self.Y[:, test_index], U_S_expected_val, self.V_mu[:,test_index], self.intercept_mu[test_index], self.gamma_v, tau_expected_val[test_index], self.S[:, test_index], self.sample_batch_fraction, self.step_size, self.SVI) for test_index in range(self.T))
	...
	def update_intercept(self):
		...
		if self.parrallel_boolean == False:
			for test_index in range(self.T):
				intercept_update_data.append(outside_update_intercept_t(intercept_mu_copy[test_index], intercept_var_copy[test_index], self.G[:, test_index], self.Y[:, test_index], self.N, U_S_expected_val, self.V_mu[:,test_index], self.F_mu[test_index], tau_expected_val[test_index], self.S[:, test_index], self.sample_batch_fraction, self.step_size, self.SVI))
		if self.parrallel_boolean == True:
				intercept_update_data = Parallel(n_jobs=self.num_test_cores)(delayed(outside_update_intercept_t)(intercept_mu_copy[test_index], intercept_var_copy[test_index], self.G[:, test_index], self.Y[:, test_index], self.N, U_S_expected_val, self.V_mu[:,test_index], self.F_mu[test_index], tau_expected_val[test_index], self.S[:, test_index], self.sample_batch_fraction, self.step_size, self.SVI) for test_index in range(self.T))
	...
	def update_tau(self):
		...
		if self.parrallel_boolean == False:
			for test_index in range(self.T):
				tau_update_data.append(outside_update_tau_t(tau_alpha_copy[test_index], tau_beta_copy[test_index], self.G[:, test_index], self.Y[:, test_index], self.N, U_S, self.V_mu[:,test_index], self.F_mu[test_index], self.intercept_mu[test_index], V_S_squared[:, test_index], F_S_squared[test_index], U_S_squared, intercept_squared[test_index], self.S[:, test_index], self.alpha_prior, self.beta_prior, self.sample_batch_fraction, self.step_size, self.SVI))
		if self.parrallel_boolean == True:
			tau_update_data = Parallel(n_jobs=self.num_test_cores)(delayed(outside_update_tau_t)(tau_alpha_copy[test_index], tau_beta_copy[test_index], self.G[:, test_index], self.Y[:, test_index], self.N, U_S, self.V_mu[:,test_index], self.F_mu[test_index], self.intercept_mu[test_index], V_S_squared[:, test_index], F_S_squared[test_index], U_S_squared, intercept_squared[test_index], self.S[:, test_index], self.alpha_prior, self.beta_prior, self.sample_batch_fraction, self.step_size, self.SVI) for test_index in range(self.T))
	...
	def initialize_variables(self):
		...
		betas, intercepts = run_linear_model_for_initialization(self.Y, self.G)
